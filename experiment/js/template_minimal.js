...
//       $("#audio_src_ogg").attr("src", 'audio/'+ stim.audio + '.ogg');
      ...
      $("#audio_player").trigger("play");
		...
		var mathproblem = stim.mathproblem + ". What is x?";
    ...
    button_done_trial : function() {
			console.log($("#recall_text").val())
	    	if (this.stim.mr == "recall") {
	    		this.response = $("#recall_text").val();
	    	} else {
	    		this.response = $("#math_text").val();	    	
	    	};
  ...
  slides.recogtrial = slide({
    name : "recogtrial",
    present: exp.recog_stims,
    ...
    present_handle: function(stim) {
      ...
      $("#error_donerecogtrial").hide();

	var recog_html = '<p style="font-size:40px"><strong>'+stim+'</strong></p>';
	$("#recognition").html(recog_html);

	var radio_html = "";
	
	for (i = 0; i < exp.choice_labels.length; i ++) {
		radio_html = radio_html+'<label><input type="radio"  name="recog_choice" value="'+exp.choice_labels[i]+'"/>'+exp.choice_labels[i]+'</label><br>';
				console.log(exp.choice_labels[i]);
	}	
	console.log(radio_html);

      $("#choice1").html(radio_html);

    },

    button_done_recogtrial : function() {
			// console.log($("#recall_text").val())
			if ($('input[name=recog_choice]:checked').length > 0) {
			      this.log_responses();
		      _stream.apply(this);
			
			} else {
				$("#error_donerecogtrial").show();
			}
    },
...
function makeStim(stim,mathp) {
	if (stim.mr == "math") {
		stim.mathproblem = mathp[c].problem
		stim.mathanswer = mathp[c].problem_answer
		c = c+1
	} else {
		stim.mathproblem = "none"
	}
	stim.audio = stim.speaker+"-"+stim.lure;
	stim.choice_labels = exp.choice_labels;
...
//   exp.structure=["i0", "consent", "instructions", "trial", 'subj_info', 'thanks'];
    exp.structure=["i0", "consent", "instructions",   "trial", "instructions2", "recogtrial", "subj_info", "thanks"];

	exp.choice_labels = _.shuffle(["Old-DeShaun", "Old-Conner", "Old", "New"]);
	
	exp.all_stims = [];
	
	stimuli = _.shuffle(stimuli);
	mathproblems = _.shuffle(mathproblems);
	
	c = 0
	for (i = 0; i < stimuli.length; i ++) {
		makeStim(stimuli[i],mathproblems);
	}
	
// 	exp.all_stims.push({audio:"conner-mountain"});
// 	exp.all_stims.push({audio:"deshaun-mountain"});
	exp.all_stims = _.shuffle(exp.all_stims);
	
	console.log(exp.all_stims);
	
	exp.recog_stims = _.shuffle(wordlist);
  ...
  exp.nQs = 50;//utils.get_exp_length(); //this does not work if there are stacks of stims (but does work for an experiment with this structure)
