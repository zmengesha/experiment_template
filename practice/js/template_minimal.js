...
//       $("#audio_src_ogg").attr("src", 'audio/'+ stim.audio + '.ogg');
    ...
    button_done_trial : function() {
	    	...
	    	if (this.stim.taskcondition == "recall") {
	    		this.response = $("#recall_textarea").val();
	    	} else {
	    		this.response = $("#math_textarea").val();	    	
	    	};
...
function makeStim(stim,mathp) {
	...
	var speakers = _.shuffle(["deshaun","conner"]);
	var taskcondition = _.shuffle(["math","recall"])[0];
	var speaker = speakers[0];

	stim.mathproblem = mathp.problem;
	stim.mathanswer = mathp.problem_answer;
	stim.audio = speaker+"-"+stim.lure;
	stim.taskcondition = taskcondition;
...
function init() {
  ...
  exp.primingStims = {"comparatives": ["John ate more food than this burger.",
	...
	for (i = 0; i < stimuli.length; i ++) {
		makeStim(stimuli[i],mathproblems[i]);
	}
